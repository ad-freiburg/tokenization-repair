...
from src.edit_distance.edit_distance import levenshtein
from src.evaluation.metrics import tp, fp, fn
from src.sequence.corruption import CorruptionType
from src.evaluation.sequence_result import SequenceResult, csv_header
from src.helper.function import current_time_string
from src.helper.files import path_exists, make_directory, write_file
...
class Evaluator:
    def __init__(self, benchmark_name=None, approach_name=None):
    ...
    def sequence_accuracy(self):
        return np.mean([result.is_correct() for result in self.sequence_results])
