...
REPLACEMENT_CHARACTERS = string.ascii_letters + string.digits + "?()~"
...
class CharAndPunctuationNoiseInducer(NoiseInducer):
    def __init__(self,
                 p: float,
                 seed: int):
        super(CharAndPunctuationNoiseInducer, self).__init__(seed)
        self.p = p

    def random_insertion_char(self):
        return self.rdm.choice(INSERTION_CHARACTERS)

    def random_replacement_char(self):
        return self.rdm.choice(REPLACEMENT_CHARACTERS)
    ...
    def apply_corruption(self,
        ...
        if corruption_type == SpellingCorruptionType.INSERTION:
            return token[:position] + self.random_insertion_char() + token[position:]
        ...
        if corruption_type == SpellingCorruptionType.REPLACEMENT:
            return token[:position] + self.random_replacement_char() + token[(position + 1):]
